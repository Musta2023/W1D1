create table students(
 student_id serial primary key,
 first_name varchar(50) not null,
 last_name varchar (50) not null,
 borth_date date not null
)
TRUNCATE TABLE students;

insert into students(first_name, last_name, borth_date) 
values 
 ('Marc', 'Benichou', '1998-11-02'),
 ('Yaon', 'Cohen', '2010-12-03'),
 ('Lea', 'Benichou', '1987-07-27'),
 ('Emilia', 'Dux', '1996-04-07'),
 ('David', 'Grez', '2003-06-14'),
 ('Omer', 'Simpson', '1980-10-03');

 select * from students

 select first_name, last_name from students

 select * from students where student_id=2

 select * from students where first_name='Marc' and  last_name='Benichou'

 select * from students where first_name='Marc' or last_name='Benichou'

 select * from students where first_name like '%a%';

 select * from students where first_name like 'A%'-- no first name begin with A

 select * from students where first_name like '%a'

 select * from students where substring (first_name from 2 )like '%a%'

 select * from students where student_id =1 and student_id=3 --no students has more than one id

 select * from students where borth_date>='2000-01-01'


 
 
 
